...
var countme=0;
		...
		{
			if (my.options[i].text=="ÆäËû" && my.options[i].selected)
			{
				countme=1;
				show(useIt1);
			}
...
function count_day()
{
	...
	if (current_month==12)
		if (current_day>20)
		{
			t_year=parseInt(current_year)+1;
			countday=t_year+"/03/20";
		}
		else
			countday=current_year+"/12/20";
	return countday;
...
function check_radio(my)
{
	
	if (my.length!=null)
	{
		if (my[0].disabled==true)
		{
			for (i=0;i<my.length-1;i++)
			{
				if (my[i].disabled==true && my[i+1].disabled!=true)
				{
					my[i+1].checked=true;
					return false;
				}	
			}
			if (my[my.length-1].disabled==true)
			{
				return false;	
			}
		}
...
//clear comma in cash
function comma_clr(my)
{
var return_me="";
	for (i=0;i<my.value.length;i++)
	{
		if (my.value.charAt(i)!=",")
		{
			return_me+=my.value.charAt(i);	
		}
	}
	return return_me;
}
...
function AddComma(num)
{
	...
	if(DotIndex!=-1)
	{
		Bignum=num.substring(0,DotIndex);	
		while(y=='0')
		{
			y=Bignum.substring(0,1);
			if(y=='0')
			{
				Bignum=Bignum.substring(1);
			}
		}
		returnNum=num.substring(DotIndex);
	}
	else
	{
		Bignum=num;
	...
	BignumLength=Bignum.length;
	countMe=Math.floor(BignumLength/3);
	
	if(countMe!=0)
	{
		for(x=1;x<countMe+1;x++)
		{
			if(x!=1)
				returnNum=Bignum.substring(BignumLength-x*3,BignumLength-(x-1)*3)+","+returnNum;
			else
				returnNum=Bignum.substring(BignumLength-x*3,BignumLength-(x-1)*3)+returnNum;
		}
		if (BignumLength!=countMe*3)
			returnNum=Bignum.substring(0,BignumLength-countMe*3)+","+returnNum;
	}
	else
		returnNum=Bignum+returnNum;
...
//if the num have comma, then clear it.
function ClearComma(num)
{
	var returnNum="";
	for(i=0;i<num.length;i++)
	{
		per=num.substring(i,i+1);
		if(per!=',')
		{
			returnNum+=per;
		}
...
function printpage(m_printpage1){
   ...
   //newwindow.document.write("<input type='button' value='´òÓ¡' onclick='window.print()'/>")
...
//
function set_currdate(m_year,m_month,m_day)
{
   var date = new Date();
   m_year.value = date.getFullYear();
   var mon = "00" + (date.getMonth()+1);
   m_month.value = mon.substring(mon.length-2,mon.length);   
   var dat = "00" + date.getDate();
   m_day.value = dat.substring(dat.length-2,dat.length);
}
//
function set_currmonth(m_year,m_month)
{
   var date = new Date();
   m_year.value = date.getFullYear();
   var mon = "00" + (date.getMonth()+1);
